...
CoordinateConvertion::CoordinateConvertion() {}

CoordinateConvertion::~CoordinateConvertion() {}

int CoordinateConvertion::LocalLBH_XYZ(double *lbh, double *xyz) {
    double a, b, e, e1, B;
...
int CoordinateConvertion::LocalXYZ_LBH(double *xyz, double *lbh) {
    ...
    Bf = X0 * (1.0 + n) / a / a0;
    while (fabs(B - Bf) > 0.00000000000005) {
        B = Bf;
        // if (X0 < 0.0) Bf = B0 - Bf;
        // else Bf = B0 + Bf;
        Bf = B0 + Bf;
...
//	else lbh[0] += 0.00000170046153;
    lbh[1] = (l + L0) * 180.0 / M_PI;
    lbh[2] = xyz[2];
    return 0;
}

int CoordinateConvertion::LBH_XYZ(double *lbh, double *xyz, double a, double b) {
    double e, N;
    e = (a * a - b * b) / (a * a);
...
int CoordinateConvertion::XYZ_LBH(double *xyz, double *lbh, double a, double b) {
    double e, N, B0, s, r;
    e = (a * a - b * b) / (a * a);
    B0 = DEF_B0 * M_PI / 180.0;
...
int CoordinateConvertion::WGS84_Local(double *WGS84) {
...
int CoordinateConvertion::Local_WGS84(double *Tokyo) {
...
int CoordinateConvertion::CenterXYZ_TangentPlaneXYZ(double *lbh0, double *xyz0, double *xyz) {
    // 地球中心直角座標 ----> 切平面直角座標
    // lbh0[0]	//地上基準点０の緯度
    // lbh0[1]	//地上基準点０の経度
    // lbh0[2]	//地上基準点０の標高
...
int CoordinateConvertion::TangentPlaneXYZ_CenterXYZ(double *lbh0, double *xyz0, double *xyz) {
    // 切平面直角座標 ----> 地球中心直角座標
    // lbh0[0]	//地上基準点０の緯度
    // lbh0[1]	//地上基準点０の経度
    // lbh0[2]	//地上基準点０の標高
...
void CoordinateConvertion::GPS84LBH_LocalXYZ(double *lbh, double *XYZ) {
    // #############################
    // Input: GPS 計測データ
    // ①lbh[0] (緯度、単位：度)
    // ②lbh[1] (経度、単位：度)
...
void CoordinateConvertion::LocalXYZ_GPS84LBH(double *XYZ, double *lbh) {
    // #############################
    // Input: GPS 計測データ
    // ①lbh[0] (緯度、単位：度)
    // ②lbh[1] (経度、単位：度)
...
void CoordinateConvertion::SetCoordinateParameters(COORDINATESYSTEMS coordsys, double lat, double lon) {
    switch (coordsys) {
    case _BEJING54:
        DEF_A2 = 6378245.0;
