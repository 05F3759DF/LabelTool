...
void CalculatePlane(int Points_Total, double *X_Coord, double *Y_Coord, double *Z_Coord,
    ...
    **	set of points.
    */
    FindScatterMatrix(Centroid, Points_Total, X_Coord, Y_Coord, Z_Coord,
                      ScatterMatrix, Order);

    /*	These next two procedures calculate the eigenvectors from the
    **	ScatterMatrix, from which the equation of the plane is found.
    **	Both these routines were coded by Penny.
    */
    tred2(ScatterMatrix, Diagonal_Matrix, Off_Diagonal_Matrix);
    tqli(Diagonal_Matrix, Off_Diagonal_Matrix, ScatterMatrix);
    /*
    ...
    */
    for (i = 0; i < 3; i++)
        Normal[Order[i]] = ScatterMatrix[i][min_index];
    /*
...
void FindScatterMatrix(double Centroid[], int Points_Total, double *X_Coord, double *Y_Coord,
    double *Z_Coord, double ScatterMatrix[3][3], int Order[]) {
    int i, TempI;
    double TempD;
    ...
    */
    ...
    for (i = 0; i < Points_Total; i++) {
        ScatterMatrix[0][0] += (X_Coord[i] - Centroid[0]) * (X_Coord[i] - Centroid[0]);
        ScatterMatrix[0][1] += (X_Coord[i] - Centroid[0]) * (Y_Coord[i] - Centroid[1]);
        ScatterMatrix[0][2] += (X_Coord[i] - Centroid[0]) * (Z_Coord[i] - Centroid[2]);
        ScatterMatrix[1][1] += (Y_Coord[i] - Centroid[1]) * (Y_Coord[i] - Centroid[1]);
        ScatterMatrix[1][2] += (Y_Coord[i] - Centroid[1]) * (Z_Coord[i] - Centroid[2]);
        ScatterMatrix[2][2] += (Z_Coord[i] - Centroid[2]) * (Z_Coord[i] - Centroid[2]);
    }
